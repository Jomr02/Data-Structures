...
ListaContigua* Grafo::getPosicionesAdyacentes(int posicionNodoOrigen) {
	...
	ListaContigua* adyacentes = new ListaContigua;

	for (int i = 0; i < Grafo::n; i++) {
		if (matriz[posicionNodoOrigen].getValor(i) != 0) {
			adyacentes->insertarAlFinal(i);
		}
...
void Grafo::convertirEnArbolRecursivo(int indiceNodoInicial, bool* vistos, Arbol* arbol) {
	assert(indiceNodoInicial >= 0);
	assert(indiceNodoInicial < Grafo::n);
	assert(vistos != nullptr);
	assert(arbol != nullptr);


	vistos[indiceNodoInicial] = true;
	ListaContigua *listaAdyacentes = getPosicionesAdyacentes(indiceNodoInicial);

	for (int i = 0; i < listaAdyacentes->getN(); i++) {
		int posicionNodo = listaAdyacentes->getValor(i);
		if (!vistos[posicionNodo]) {
			arbol->insertar(nombresNodos.at(posicionNodo), nombresNodos.at(indiceNodoInicial));

			convertirEnArbolRecursivo(posicionNodo, vistos, arbol);
		}

	}

	free(listaAdyacentes);
...
Arbol* Grafo::convertirEnArbol(char raiz) {
	...
	int posicionNodo = nombresNodos.find(raiz, 0);

	Arbol* arbol = new Arbol(raiz);

	bool* vistos = (bool*)malloc(Grafo::n * sizeof (bool));
...
string Grafo::recorrerAnchura(char nodoInicial) {
	...
	bool* visitados = (bool*)malloc (Grafo::n * sizeof(bool));
	...
	string recorrido = "";
	char* colaNodos = (char*)malloc(Grafo::n * sizeof (char));
	int inicioCola = -1;
	int finalCola = 0;
	colaNodos[0] = nodoInicial;
	int nCola = 1;
	...
	while (nCola > 0) {
		...
		inicioCola++;
		nodoInicial = colaNodos[inicioCola];
		visitados[nombresNodos.find(nodoInicial, 0)] = true;
		nCola--;
		recorrido.append(1 , nodoInicial);

		ListaContigua* adyacentes = getPosicionesAdyacentes(nombresNodos.find (nodoInicial, 0));

		for (int i = 0; i < adyacentes->getN(); i++) {
			if (!visitados[adyacentes->getValor(i)]) {

				visitados[adyacentes->getValor(i)] = true;
				finalCola++;
				colaNodos[finalCola] = nombresNodos.at(adyacentes->getValor(i));
				nCola++;
...
string Grafo::recorrerTopologico() {
	...
	for (int i = 0; i < Grafo::n; i++) {
		if (getGradoEntrada(i) == 0) {
			recorrido.append(1, nombresNodos.at(i));
			borrarNodo(nombresNodos.at(i));
			recorrido.append(recorrerTopologico());
		}
...
int main() {
	...
	matriz.borrarArco('C', 'C');
	matriz.borrarArco('D', 'D');
	matriz.imprimir();
	...
	matriz.convertirEnArbol('C')->imprimir();
	matriz.convertirEnArbol('G')->imprimir();
	matriz.convertirEnArbol('H')->imprimir();


	cout << matriz.recorrerAnchura('D') << endl;
	...
	cout << grafoNumeros.recorrerTopologico() << endl;
