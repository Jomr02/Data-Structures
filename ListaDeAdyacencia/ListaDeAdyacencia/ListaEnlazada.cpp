...
Nodo * ListaEnlazada::getNodo (int posicion) {
	...
	// Si el nodo no es contiguo al ultimo accedido, y se llega antes a él iterando hacia atrás	
		...
		resultado = punteroAPrimerNodo->anteriorNodo; // Hacemos que el resultado apunte al último nodo
...
Ruta ListaEnlazada::getValor(int posicion) {
	assert(n > 0);
	assert (posicion >= 0 && posicion <= n-1); 
...
void ListaEnlazada::setValor(int posicion, Ruta nuevoValor) {
	assert(n > 0);
	assert (posicion >= 0 && posicion <= n-1); 
...
void ListaEnlazada::insertar (int posicion, Ruta nuevoValor) {
...
void ListaEnlazada::concatenar(ListaEnlazada *listaAConcatenar) {
	...
	for (int i = 0; i < m; i++) {
		Ruta elementoACopiar = listaAConcatenar->getValor(i);
		insertar(n, elementoACopiar);
	}
}

int ListaEnlazada::buscar(string clave) {
	...
	while (!encontrado && posicion < n) {
		...
		if (*(getValor(posicion).aeropuerto) == clave) encontrado = true;
		posicion++;
	}
