...
string Grafo::recorridoProfundidad(char nodo) {
	assert(existeNodo(nodo));

	int posicionDelNodo = posicionNodo(nodo);
	...
	recorrido.append(recorridoProfundidadRecursivo(posicionDelNodo, vistos));
...
string Grafo::recorridoProfundidadRecursivo(int posicionNodo, bool* vistos) {
	assert(posicionNodo >= 0);
	assert(posicionNodo < n);
	assert(vistos != NULL);
	...
	for (int i = 0; i < n; i++) {
		if (existeArco(nodos.at(posicionNodo), nodos[i]) && !vistos[i]) {
