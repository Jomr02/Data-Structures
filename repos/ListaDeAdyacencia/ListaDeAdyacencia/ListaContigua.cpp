...
Aeropuerto ListaContigua::getValor(int posicion) {
	assert(posicion >= 0 && posicion <= n-1);
	return (vector[posicion]);
}

void ListaContigua::setValor(int posicion, Aeropuerto nuevoValor) {
	assert(posicion >= 0 && posicion <= n-1);
...
void ListaContigua::insertarAlFinal (Aeropuerto nuevoValor) {
...
void ListaContigua::eliminar(int posicion) {
	...
	memmove(&vector[posicion], &vector[posicion + 1], sizeof(Aeropuerto)*(n - posicion - 1));
	n--;
...
void ListaContigua::concatenar(ListaContigua *listaAConcatenar) {
	...
	vector = (Aeropuerto*)realloc(vector, sizeof(Aeropuerto)*(capacidad + m));
	capacidad += m;
...
int ListaContigua::buscar(string clave) {
	...
	while (!encontrado && posicion < n) {
		if (*(vector[posicion].nombre) == clave) encontrado = true;
		posicion++;
	}
