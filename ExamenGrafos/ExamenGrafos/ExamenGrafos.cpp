...
ListaContigua* Grafo::getPosicionesAdyacentes(int posicionNodoOrigen) {
	assert(posicionNodoOrigen >= 0 && posicionNodoOrigen < n);
...
void Grafo::convertirEnArbolRecursivo(int indiceNodoInicial, bool* vistos, Arbol* arbol) {
	assert(indiceNodoInicial >= 0 && indiceNodoInicial < n);
	assert(vistos != NULL);
	assert(arbol != NULL);

	vistos[indiceNodoInicial] = true;

	ListaContigua* posicionesAdyacentes = getPosicionesAdyacentes(indiceNodoInicial);

	

	for (int i = 0; i < posicionesAdyacentes->getN(); i++) {
		if (!vistos[posicionesAdyacentes->getValor(i)]) {
			arbol->insertar(nombresNodos[posicionesAdyacentes->getValor(i)], nombresNodos[indiceNodoInicial]);
			convertirEnArbolRecursivo(posicionesAdyacentes->getValor(i), vistos, arbol);
		}
...
Arbol* Grafo::convertirEnArbol(char raiz) {
	...
	int indiceInicial = obtenerIndice(raiz);
	...
	convertirEnArbolRecursivo(indiceInicial, vistos, arbol);
...
string Grafo::recorrerAnchura(char nodoInicial) {
	...
	while (nCola > 0) {
		char letra = cola[inicioCola];

		recorrido.append(1, letra);
		inicioCola++;
		nCola--;

		ListaContigua *adyacentes = getPosicionesAdyacentes(obtenerIndice(letra));
